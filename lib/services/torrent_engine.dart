...
class TorrentEngine {
  ...
  Future<void> start(String magnetLink) async {
    ...
    try {
      ...
      // Start the stream immediately — this matches the plugin's own
      // documented usage (addMagnet then startStream, back to back).
      // Earlier this waited for t.hasMetadata from torrentUpdates first,
      // which deadlocked: with streamOnly:true the native engine appears
      // to gate real session activity — including metadata fetch — behind
      // an actual stream reader existing. Waiting for metadata to start
      // the stream, when the stream is what triggers metadata fetch,
      // meant it sat at "resolving" forever. startStream() itself reports
      // progress via StreamState (idle → buffering → ready), so there's
      // no need to gate on metadata separately.
      _startStream(_activeTorrentId!);
  ...
  void _onTorrentUpdate(Map<int, lt.TorrentInfo> torrents) {
    ...
    // Torrent-level updates now only feed the peers/speed HUD while the
    // stream is still buffering — actual phase transitions come from
    // _onStreamUpdate below, since startStream() runs unconditionally in
    // start() rather than being gated on metadata here.
    if (_current.phase == TorrentPhase.resolving ||
        _current.phase == TorrentPhase.buffering) {
      _emit(_current.copyWith(peers: t.numPeers));
    }
  }
