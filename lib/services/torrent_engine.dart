...
//   • Explicitly awaits TrackerManager.fetchBestTrackers() (ngosang/
//     trackerslist) before addMagnet() — the plugin's own internal fetch
//     is fire-and-forget and can otherwise race addMagnet()
//   • Waits for real metadata (t.hasMetadata) before calling startStream() —
//     the native layer throws if called too early, it does not wait
//     internally. A 45s timeout surfaces a clear error if peers never
//     connect at all
//   • streamOnly: true — only downloads what the reader actually needs,
//     instead of pulling the whole file in the background. IMPORTANT:
//     this adds the torrent PAUSED natively — resumeTorrent() must be
//     called right after addMagnet() or it never does any networking
//     at all (this was the actual root cause of a long "stuck at
//     resolving, -1 peers forever" bug — see resumeTorrent() call below)
...
class TorrentState {
  ...
  // ── Raw native diagnostics — surfaced directly from the plugin's own
  // TorrentInfo struct, unfiltered, for debugging the plugin itself
  // rather than our simplified phase/peers summary above.
  final String nativeState; // t.state.name — the plugin's own enum, not ours
  final int numSeeds;
  final bool isPaused;
  final int queuePosition;
  final String libraryVersion; // e.g. "2.0.11" — confirms the native .so loaded
...
class TorrentEngine {
    ...
    try {
      ...
      if (!lt.LibtorrentFlutter.isInitialized) {
        await lt.LibtorrentFlutter.init(
          ...
          // fetchTrackers here is fire-and-forget internally — we still
          // explicitly await TrackerManager.fetchBestTrackers() below so
          // addMagnet() never races an empty tracker list.
          fetchTrackers: true,
          pollInterval: const Duration(milliseconds: 500),
        );

        // NOTE: previously forced disableUtp:true here on a theory that
        // UDP was being blocked on-device. A direct Termux test with
        // aria2c on the SAME magnet, SAME network, VPN off, got 44 peer
        // connections and pulled metadata in ~8s — proving the network
        // is fine and uTP/UDP work perfectly here. That override was
        // solving a problem that didn't exist, and risked forcing a
        // less-tested TCP-only code path in this plugin. Removed —
        // leave the engine on its real defaults.
      }

      // Explicitly wait for the best-tracker list (ngosang/trackerslist,
      // live-updated) rather than relying on init()'s fire-and-forget
      // fetch, which can otherwise still be in flight when addMagnet()
      // runs below — that race meant it silently injected zero extra
      // trackers on a fresh session.
      await lt.TrackerManager.fetchBestTrackers();

      final engine = lt.LibtorrentFlutter.instance;
      final libVersion = engine.libraryVersion;
      debugPrint('[TorrentEngine] libtorrent version: $libVersion');
      _emit(_current.copyWith(libraryVersion: libVersion));
      ...
      debugPrint('[TorrentEngine] addMagnet returned id=$_activeTorrentId');

      // THE ACTUAL BUG: streamOnly:true adds the torrent in a PAUSED
      // state natively (confirmed via debug output — "checkingFiles
      // (paused)" forever, peers frozen at a sentinel -1). A paused
      // torrent does zero networking — no DHT, no tracker announces, no
      // peer connections — so nothing was ever going to arrive no matter
      // how long we waited or what trackers/transport we configured.
      // Every earlier fix in this file was chasing symptoms of this one
      // missing call.
      engine.resumeTorrent(_activeTorrentId!);
      debugPrint('[TorrentEngine] resumeTorrent called for id=$_activeTorrentId');

      _torrentSub = engine.torrentUpdates.listen(_onTorrentUpdate);
      _streamSub = engine.streamUpdates.listen(_onStreamUpdate);

      // Wait for real metadata before calling startStream() — the native
      // layer throws "no metadata yet" if called too early, it does not
      // wait internally. A previous version of this file called
      // startStream() immediately based on a misread of the plugin's
      // usage example; that was wrong, this direct error confirmed it.
      //
      // A 45s timeout is here so a torrent with dead/unreachable trackers
      // (or a genuinely bad network) surfaces a clear error instead of an
      // infinite "resolving" spinner — see stream_service.dart's tracker
      // list for the actual cause of that stall (UDP-only trackers).
  ...
  void _onTorrentUpdate(Map<int, lt.TorrentInfo> torrents) {
    ...
    if (t == null) {
      // Torrent ID isn't showing up in the plugin's own status map at
      // all — worth knowing about explicitly rather than silently
      // sitting at "resolving" with stale data.
      debugPrint('[TorrentEngine] torrentId=$_activeTorrentId not found '
          'in torrentUpdates (${torrents.length} torrents known)');
      return;
    }

    debugPrint('[TorrentEngine] raw status: name="${t.name}" '
        'state=${t.state.name} hasMetadata=${t.hasMetadata} '
        'numPeers=${t.numPeers} numSeeds=${t.numSeeds} '
        'isPaused=${t.isPaused} queuePos=${t.queuePosition} '
        'progress=${t.progress} totalWanted=${t.totalWanted}');

    if (t.state == lt.TorrentState.error) {
      _emit(_current.copyWith(
        phase: TorrentPhase.error,
        errorMessage: t.errorMsg.isNotEmpty ? t.errorMsg : 'Torrent error',
        nativeState: t.state.name,
      ));
      return;
    }

    // Once real metadata (the file list) is in, kick off the stream —
    // startStream() throws if called before this is true, it does not
    // wait internally. _activeStreamId null-check guards against calling
    // it twice across repeated torrentUpdates ticks.
    if (t.hasMetadata && _activeStreamId == null) {
      _metadataTimer?.cancel();
      _startStream(t.id);
    }
