...
//   • disableUtp: true via configureSession() — forces TCP-only for real
//     peer data connections, since uTP (the default) rides on UDP and can
//     fail silently on networks that block UDP broadly, even after DHT/
//     trackers succeed
  ...
  Future<void> start(String magnetLink) async {
    ...
    try {
      ...
      if (!lt.LibtorrentFlutter.isInitialized) {
        ...
        // Force TCP-only for actual peer data connections. DHT and
        // trackers can succeed over HTTP/TCP, but the default transport
        // for real piece data is uTP — which rides on UDP. On a network
        // that blocks UDP broadly, that would fail silently even after
        // peers are discovered. disableUtp forces plain TCP instead.
        lt.LibtorrentFlutter.instance.configureSession(
          const lt.BtConfig(disableUtp: true, disableDht: false),
        );
      }
